...
class Repeat(object):
    '''Repeats the channels of grayscale image'''
    def __init__(self):
        pass
...
def get_transforms(size):
    image_transforms = transforms.Compose([transforms.Resize((size, size), Image.ANTIALIAS),
                                           transforms.ToTensor(),
                                           Repeat(),
                                           transforms.Normalize(mean=mean_train, std=std_train)])
    gt_transforms = transforms.Compose([transforms.Resize((size,size)),
                                        transforms.ToTensor()])

    return image_transforms, gt_transforms
